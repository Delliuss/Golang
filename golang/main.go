...
func addCharacterHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodPost {
		...
		price, err := strconv.Atoi(r.FormValue("price")) // Преобразуем цену в int
		if err != nil {
		...
		// Перенаправляем на страницу профиля после добавления персонажа
...
func main() {
	...
	http.HandleFunc("/add-character", addCharacterHandler) // Обработчик для добавления персонажа
